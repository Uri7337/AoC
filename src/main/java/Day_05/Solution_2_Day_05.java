...
public class Solution_2_Day_05 {
		...
		for (int i = 0; i < file.size(); i++) {
			...
			if (line.contains("seeds:")) {
				...
				if (line.contains("-to-")) {
					...
					// numbers
					// add then to the last category in the list (their category will always be the
					...
					if (td.s.state.equals(cl.from)) {
						...
						if (td.s.rangeStart < r.sourceRangeStart) {
							...
							if (td.s.rangeEnd >= r.sourceRangeStart && td.s.rangeEnd <= r.sourceRangeEnd) {
								...
								seedList.add(td.sB);
								break;
							}else if (td.s.rangeEnd > r.sourceRangeEnd) {
								// vpravo větší
								// 79 - 110
								// 98 - 100
								// rozlomit na 3 seedy
								...
								seedList.add(td.sC);
								break;
							}else if (td.s.rangeEnd < r.sourceRangeStart) {
								// menší i vpravo
								// 79 - 92
								// 98 - 99
						...
						}else if (td.s.rangeStart >= r.sourceRangeStart && td.s.rangeStart <= r.sourceRangeEnd) {
	...
	class Seed {
		// changing state based on where the seed is currently located e.g. if seed ->
		// soil state:soil
		String state;
		long number;
