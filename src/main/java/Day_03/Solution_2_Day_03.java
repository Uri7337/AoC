...
public class Solution_2_Day_03 {
	...
	public Object getSolution(String filepath) {
		...
		// ep.p(line);
		String[] matches = patternFinder(line, "mul\\(\\d+,\\d+\\)");
		 ...
		 * if number is lower than my mul index
		 * and has the smallest difference in the whole array (let say do)
		 * and also smaller than number from the second array
		 * so i ve got the number i needed and if it s do or dont.
		 * then im gonna find the number in that arraylist if needed by .contains
		 * AND in mul index is smaller than the first number of both arrays it is do
		 * 
		 */
	...
	String[] patternFinder(String line, String pattern) {
	...
	void findAll(String line, String pattern, int index, int end, ArrayList<Integer> bob) {
		if (index < end) {
